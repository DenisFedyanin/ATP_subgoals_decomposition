...
- `HOLE_FILLING`: замена одной целевой `sorry`-заглушки внутри MCSP/semi-proof каркаса. Один MCSP job соответствует одной целевой `sorry`-заглушке, которую search пытается заменить. родительский semi-proof может содержать несколько `sorry`.
- `PREFIX_COMPLETION`: продолжение уже валидного proof prefix-а до полного Lean-доказательства. Один prefix job соответствует одному валидному префиксу доказательства, который надо продолжить suffix-ом.

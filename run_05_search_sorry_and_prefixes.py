#!/usr/bin/env python3
"""
Запускает encoder-guided Lean tree search для задач, подготовленных run_04_mine_failed_problems.py
Для каждой задачи используется verifier-in-the-loop search:
генератор предлагает тактики, encoder ранжирует пары `(state, tactic)`,
k-DPP выбирает разнообразный subset, а Lean проверяет переходы.
"""
...
class GeneratorConfig:
    ...
    top_p: float = 0.95
    raw_candidates_per_node: int = 16
...
@dataclass
class EncoderConfig:
    checkpoint_path: Optional[str] = "outputs/encoder_runs/transition_encoder_v1/checkpoint_best.pt"
    ...
    w_run02_utility: float = 1.0
    w_run02_validity: float = 0.6
    w_run02_closure: float = 0.8
    w_run02_delta_decrease: float = 0.3
    w_run02_error_penalty: float = -0.8
    w_run02_timeout_penalty: float = -0.5


@dataclass
class KDPPConfig:
    enabled: bool = True
    raw_pool_size: int = 16
...
class SearchConfig:
    ...
    prefix_completion: SearchBudgetConfig = field(default_factory=lambda: SearchBudgetConfig(
        max_nodes_per_job=512, max_depth=16, timeout_sec=120, max_block_lines=3
    ))
...
def labels_from_mapping(mapping: Dict[str, Any], id_key: str, name_key: str, default: List[str]) -> List[str]:
    by_id = mapping.get(id_key)
...
def build_encoder_pair_text(state: str, tactic: str) -> str:
    return f"<STATE>\n{state}\n\n<TACTIC>\n{tactic}\n"
...
class Run02CheckpointBackend:
    ...
    def __init__(self, checkpoint_path: str | Path, cfg: EncoderConfig):
        ...
        try:
            ...
            raise RuntimeError("Loading a run_02 checkpoint requires torch.") from e
        self.torch = torch
        requested_device = cfg.device
        if requested_device == "cuda" and not torch.cuda.is_available():
            log("encoder cuda requested but unavailable; falling back to cpu", "warning")
        ...
        if missing or unexpected:
            log(f"encoder checkpoint load warnings: missing={missing[:8]} unexpected={unexpected[:8]}", "warning")
        ...
        log(f"loaded run_02 checkpoint backend from {self.checkpoint_path}")
    ...
    def predict_batch(self, texts: List[str]) -> List[Dict[str, Any]]:
        ...
        use_autocast = self.device.type == "cuda" and self.precision in {"bf16", "fp16", "float16"}
        with torch.no_grad():
        ...
        embedding = out["embedding"].detach().float().cpu().numpy()
        ...
        for i in range(len(texts)):
            ...
            rows.append({
                ...
                "embedding": [float(x) for x in embedding[i].tolist()],
            })
        return rows
...
class EncoderScorer:
    def __init__(self, cfg: EncoderConfig):
        ...
        self.backend: Optional[Run02CheckpointBackend] = None
        if self.fn is None and cfg.checkpoint_path and Path(cfg.checkpoint_path).exists():
            try:
                self.backend = Run02CheckpointBackend(cfg.checkpoint_path, cfg)
            except Exception as e:
                if not cfg.heuristic_fallback:
                    raise
                log(f"could not load run_02 checkpoint backend, using heuristic encoder fallback: {e}", "warning")
    ...
    def score_and_embed(self, state: str, candidates: List[Candidate], job: Dict[str, Any]) -> List[Candidate]:
        ...
        if self.backend is not None:
            texts = [build_encoder_pair_text(state, c.text) for c in candidates]
            raw = self.backend.predict_batch(texts)
            return self._apply_run02_output(candidates, raw)
    ...
    def _apply_run02_output(self, candidates: List[Candidate], raw: List[Dict[str, Any]]) -> List[Candidate]:
        for i, c in enumerate(candidates):
            pred = raw[i] if i < len(raw) else {}
            score = (
                self.cfg.w_run02_utility * safe_float(pred.get("utility_score"), 0.0)
                + self.cfg.w_run02_validity * safe_float(pred.get("validity_prob"), 0.5)
                + self.cfg.w_run02_closure * safe_float(pred.get("closure_prob"), 0.0)
                + self.cfg.w_run02_delta_decrease * safe_float(pred.get("delta_decrease_prob"), 0.0)
                + self.cfg.w_run02_error_penalty * safe_float(pred.get("error_prob"), 0.0)
                + self.cfg.w_run02_timeout_penalty * safe_float(pred.get("timeout_prob"), 0.0)
            )
            c.encoder_score = float(score)
            emb = pred.get("embedding")
            c.embedding = [float(x) for x in emb] if isinstance(emb, list) and emb else hashing_embedding(c.text, self.cfg.embedding_dim)
            c.quality = squash_quality(c.encoder_score)
            c.metadata["encoder_prediction"] = {k: v for k, v in pred.items() if k != "embedding"}
        return candidates
...
def select_by_k_dpp(candidates: List[Candidate], cfg: KDPPConfig) -> List[Candidate]:
    if not candidates:
        return []
    candidates = sorted(candidates, key=lambda c: c.quality, reverse=True)[:cfg.raw_pool_size]
    if not cfg.enabled:
        return candidates[:cfg.select_k]
    try:
...
def config_summary(cfg: RunConfig, jobs: List[Dict[str, Any]]) -> Dict[str, Any]:
    warnings: List[str] = []
    mcsp_path = Path(cfg.inputs.mcsp_jobs_path)
    prefix_path = Path(cfg.inputs.prefix_jobs_path)
    ckpt_path = Path(cfg.encoder.checkpoint_path) if cfg.encoder.checkpoint_path else None
    if not mcsp_path.exists():
        warnings.append(f"missing_mcsp_jobs: {mcsp_path}")
    if not prefix_path.exists():
        warnings.append(f"missing_prefix_jobs: {prefix_path}")
    if not jobs:
        warnings.append("no_search_jobs_loaded")
    if cfg.generator.engine == "none" and not cfg.experiment.dry_run:
        warnings.append("generator_engine_none: real search requires --generator-engine vllm or --generator-adapter")
    if cfg.encoder.adapter_path is None and (ckpt_path is None or not ckpt_path.exists()):
        if cfg.encoder.heuristic_fallback:
            warnings.append("encoder_checkpoint_missing: search will use heuristic fallback unless a run_02 checkpoint or adapter is provided")
        else:
            warnings.append("encoder_checkpoint_missing_and_heuristic_fallback_false")
    return {
        "experiment_name": cfg.experiment.experiment_name,
        "created_at": now_ts(),
        "inputs": {
            "mcsp_jobs_path": str(mcsp_path),
            "mcsp_jobs_exists": mcsp_path.exists(),
            "mcsp_jobs_count_raw": count_jsonl(mcsp_path),
            "prefix_jobs_path": str(prefix_path),
            "prefix_jobs_exists": prefix_path.exists(),
            "prefix_jobs_count_raw": count_jsonl(prefix_path),
            "frontier_path": cfg.inputs.frontier_path,
        },
        "loaded_jobs": {
            "total": len(jobs),
            "hole_filling": sum(1 for j in jobs if normalize_job_type(j) == "HOLE_FILLING"),
            "prefix_completion": sum(1 for j in jobs if normalize_job_type(j) == "PREFIX_COMPLETION"),
            "job_type_filter": cfg.experiment.job_type_filter,
            "max_jobs": cfg.experiment.max_jobs,
        },
        "generator": {
            "engine": cfg.generator.engine,
            "model_name": cfg.generator.model_name,
            "raw_candidates_per_node": cfg.generator.raw_candidates_per_node,
            "max_new_tokens_per_candidate": cfg.generator.max_new_tokens_per_candidate,
        },
        "encoder": {
            "adapter_path": cfg.encoder.adapter_path,
            "checkpoint_path": cfg.encoder.checkpoint_path,
            "checkpoint_exists": bool(ckpt_path and ckpt_path.exists()),
            "heuristic_fallback": cfg.encoder.heuristic_fallback,
            "device": cfg.encoder.device,
            "precision": cfg.encoder.precision,
        },
        "k_dpp": dataclasses.asdict(cfg.k_dpp),
        "search_budget": {
            "hole_filling": dataclasses.asdict(cfg.search.hole_filling),
            "prefix_completion": dataclasses.asdict(cfg.search.prefix_completion),
            "expansions_per_node": cfg.search.expansions_per_node,
        },
        "warnings": warnings,
...
def main() -> None:
    ...
    if cfg.experiment.config_only:
        out_dir = ensure_dir(cfg.outputs.out_dir)
        summary = config_summary(cfg, jobs)
    ...
    if not jobs:
        raise RuntimeError(
            "No run_05 search jobs loaded. Run run_04_mine_failed_problems.py first, "
            "or pass --mcsp-jobs/--prefix-jobs to existing JSONL files."
        )

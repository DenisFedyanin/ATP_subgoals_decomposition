...
CONFIG: Dict[str, Any] = {
    ...
    "offline_encoder_eval": {
        ...
        "required_columns": [
            "state_pp", "tactic", "sibling_group_id", "y", "result_type",
...
def utility_target(row: Dict[str, Any]) -> float:
    ...
    if result_type == "ProofFinished" or int(row.get("m_closed", 0) or 0) == 1:
        return 4.0
    if y == 0:
        c_err = str(row.get("c_err", "") or "")
        return -2.0 if c_err == "timeout" else -1.0
    d = row.get("delta_n_goals", None)
    if d is None or pd.isna(d):
        return 0.5
    d = int(d)
...
class EncoderAdapter:
    ...
    def _load_backend(self) -> None:
        ...
        raise RuntimeError(
            "No encoder inference backend found."
        )
...
def run_offline_encoder_eval(config: Dict[str, Any], encoder: EncoderAdapter, out_dir: Path) -> Dict[str, Any]:
    ...
    df = theorem_level_holdout(df, cfg["heldout_theorem_ratio"], cfg["sample_seed"], cfg["split_by"])
    ...
    metrics: Dict[str, Any] = {
        ...
        "tau_log_rmse": rmse(tau_true_log, tau_pred_log),
        "tau_log_mae": mae(tau_true_log, tau_pred_log),
    }
...
def apply_gentle_pruning(
    ...
    for i, p in enumerate(preds):
        ...
        if p is None:
            keep.add(i)
            continue
        if candidates[i].generator_rank < int(cfg.get("never_drop_if_generator_rank_lte", 0)):
            keep.add(i)
            continue
...
def run_search_eval(config: Dict[str, Any], generator: GeneratorAdapter, encoder: Optional[EncoderAdapter], out_dir: Path, cli_max_theorems: Optional[int]) -> Dict[str, Any]:
    ...
    for ti, thm in enumerate(theorems):
        ...
        for mode in modes:
            result, attempts, proofs = run_search_for_theorem(thm, mode, generator, encoder, config)
...
def summarize_search_results(df: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for mode, g in df.groupby("search_mode"):
        ...
        per_mode.append({
            ...
            "mean_valid_tactic_rate": safe_mean((g["valid_tactics"] / g["tactics_executed"].replace(0, np.nan)).tolist()),
            "mean_timeout_rate": safe_mean((g["timeouts"] / g["tactics_executed"].replace(0, np.nan)).tolist()),
        })
    comp = {}
    ...
    return {"per_mode": per_mode, "comparison": comp}
...
def main() -> None:
    ...
    out_dir = ensure_dir(config["output"]["save_dir"])
    write_json(out_dir / "run_config.json", config)

    encoder = None
    if not args.skip_encoder and not args.search_only:

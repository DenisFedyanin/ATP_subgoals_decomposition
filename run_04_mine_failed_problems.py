...
class DatasetConfig:
    ...
    include_symbols: List[str] = field(default_factory=lambda: ["ℝ", "Real", "≤", "<", "≥", ">", "^2", "abs"])
...
@dataclass
class IOConfig:
    theorems_jsonl: str = "data/theorems.jsonl"
    attempts_jsonl: Optional[str] = None
...
def load_theorems(path: str, cfg: RunConfig) -> List[TheoremRecord]:
    ...
    for rec in read_jsonl(path):
        tid = str(rec.get(cfg.dataset.theorem_id_key) or rec.get("theorem_id") or stable_id(str(rec), prefix="thm"))
        formal = str(rec.get(cfg.dataset.formal_statement_key) or rec.get("formal") or rec.get("statement") or "").strip()
        if not formal:
            continue
        nl = str(rec.get(cfg.dataset.natural_language_key) or rec.get("nl_statement") or rec.get("problem") or "")
...
def load_attempts(path: str, cfg: RunConfig) -> Dict[str, List[ProofAttempt]]:
    ...
    for rec in read_jsonl(path):
        theorem_id = str(rec.get("theorem_id") or rec.get(cfg.dataset.theorem_id_key) or "")
        if not theorem_id:
            continue
        proof_text = ""
        for key in ["proof_text", "generated_proof", "proof", "tactic", "completion"] + cfg.dataset.proof_key_candidates:
            if rec.get(key):
                proof_text = str(rec[key])
                break
        if not proof_text:
            continue
        aid = str(rec.get("attempt_id") or stable_id(theorem_id, proof_text, prefix="attempt"))
        attempts.setdefault(theorem_id, []).append(
            ProofAttempt(aid, theorem_id, proof_text, str(rec.get("mode") or "input"), str(rec.get("source") or "input"), rec)
        )
    return attempts
...
class Miner:
    def __init__(self, cfg: RunConfig):
        ...
        self.generator = make_generator(cfg) if (cfg.full_generation.enabled or cfg.mcsp.enabled or cfg.prefix_fallback.enabled) else NoOpGenerator()
    ...
    def process_mcsp(self, tr: TheoremRecord, attempt: ProofAttempt, style: ProofStyle) -> None:
        ...
        for hole in holes:
            ...
            try:
                ...
                self.write_hole_record(tr, hole, "UNRESOLVED_NO_GENERATOR", semi_ver, None, [])
                return
    ...
    def process_prefix_fallback(self, tr: TheoremRecord, attempt: ProofAttempt, style: ProofStyle) -> None:
        ...
        for prefix in prefixes:
            ...
            try:
                ...
                self.write_prefix_record(tr, prefix, None, [], "UNRESOLVED_NO_GENERATOR")
                return
            self.report["total_lean_calls_approx"] += len(sols)
            status = "SOLVED" if stats.solved_count > 0 else ("PARTIAL_PROGRESS" if stats.valid_partial_count > 0 else "UNRESOLVED")
...
def main() -> None:
    ...
    if cfg.experiment.dry_run:
        print(json.dumps(dataclasses.asdict(cfg), ensure_ascii=False, indent=2))
        return
    miner = Miner(cfg)
    miner.run()

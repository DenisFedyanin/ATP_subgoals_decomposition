...
def commit_git_snapshot(path: str | Path, message: str) -> str:
    path = Path(path)
    run_cmd("git add .", cwd=path, check=False)
    run_cmd(f'git commit -m "{message}" || true', cwd=path, check=False)
    return get_git_commit(path)
...
class TupleWriter:
    ...
    def add(self, row: Dict[str, Any]) -> None:
        ...
        self.stats[f"dataset_name::{row.get('dataset_name')}"] += 1
        self.stats[f"generation_pass::{row.get('generation_pass')}"] += 1
        self.stats[f"depth::{row.get('depth')}"] += 1
...
""".strip() + "\n",
    ...
    commit = commit_git_snapshot(repo_dir, "materialize lean project")
    return commit
...
class TupleGenerationRun:
    def __init__(self, args: argparse.Namespace):
        ...
        if args.target_tuples is not None:
            CONFIG["dataset_size"]["target_total_attempt_tuples"] = args.target_tuples
        if args.output_dir:
            CONFIG["output"]["root_dir"] = args.output_dir
        if args.max_workbook_rows is not None:
            CONFIG["dataset_sources"]["lean_workbook_local"]["max_rows_to_materialize"] = args.max_workbook_rows
        ...
        self.target = CONFIG["dataset_size"]["target_total_attempt_tuples"]
        self.root = Path(CONFIG["output"]["root_dir"])
        if self.root.exists() and any(self.root.iterdir()) and not args.overwrite_output_dir:
            raise RuntimeError(
                f"Output directory is not empty: {self.root}. "
                "Use --overwrite-output-dir or pass a fresh --output-dir to avoid mixing shards."
            )
        if self.root.exists() and args.overwrite_output_dir:
    ...
    def dry_run_summary(self) -> Dict[str, Any]:
        sources = {
            "synthetic_small_local": CONFIG["dataset_sources"]["synthetic_small_local"],
            "lean_workbook_local": CONFIG["dataset_sources"]["lean_workbook_local"],
            "external_manifests": CONFIG["dataset_sources"]["external_manifests"],
        }
        max_attempts_per_theorem = CONFIG["tuple_search"]["max_attempt_tuples_per_theorem"]
        enabled_sources = []
        estimated_theorems_upper_bound = 0
        if sources["synthetic_small_local"]["enabled"]:
            n_synthetic = len(make_synthetic_theorems())
            estimated_theorems_upper_bound += n_synthetic
            enabled_sources.append({
                "name": "synthetic_small_local",
                "configured_theorems": n_synthetic,
                "weight": sources["synthetic_small_local"]["weight"],
            })
        if sources["lean_workbook_local"]["enabled"]:
            n_workbook = sources["lean_workbook_local"]["max_rows_to_materialize"]
            estimated_theorems_upper_bound += n_workbook
            enabled_sources.append({
                "name": "lean_workbook_local",
                "configured_max_rows": n_workbook,
                "weight": sources["lean_workbook_local"]["weight"],
            })
        for m in sources["external_manifests"]:
            if m.get("enabled"):
                enabled_sources.append({
                    "name": m.get("dataset_name"),
                    "manifest_path": m.get("path"),
                    "weight": m.get("weight"),
                })
        ...
        summary = {
            ...
            "target_tuples": self.target,
            "max_attempts_per_theorem": max_attempts_per_theorem,
            "estimated_theorems_upper_bound": estimated_theorems_upper_bound,
            "estimated_tuple_capacity_upper_bound": estimated_theorems_upper_bound * max_attempts_per_theorem,
            "enabled_sources": enabled_sources,
            "writer_outputs": {
                "jsonl": CONFIG["output"]["write_jsonl"],
                "parquet": CONFIG["output"]["write_parquet"],
                "shard_size_tuples": CONFIG["output"]["shard_size_tuples"],
            },
            "encoder_required_columns_present": [
                "theorem_id", "theorem_name", "state_pp", "tactic", "tactic_head",
                "sibling_group_id", "y", "result_type", "n_goals_before",
                "n_goals_after", "delta_n_goals", "m_closed", "m_new",
                "m_decreased", "c_err", "tactic_rank", "tactic_score",
                "state_hash", "next_state_hash", "is_duplicate_state_tactic",
                "is_duplicate_exact_tuple",
            ],
        }
        save_json(self.root / "dry_run_summary.json", summary)
        print("[dry_run]", json.dumps(summary, ensure_ascii=False, indent=2))
        return summary
    ...
    def run(self) -> None:
        ...
        theorem_count = sum(len(v) for v in pools.values())
        max_possible_attempts = theorem_count * CONFIG["tuple_search"]["max_attempt_tuples_per_theorem"]
        print(f"[run] theorem pools loaded: {theorem_count} theorem specs")
        print("[run] theorem pools by source:", json.dumps({k: len(v) for k, v in pools.items()}, ensure_ascii=False))
        print(f"[run] target tuples: {self.target}")
        if max_possible_attempts < self.target:
            print(
                f"[warning] target tuples may be unreachable: target={self.target}, "
                f"theorems={theorem_count}, max_attempts_per_theorem={CONFIG['tuple_search']['max_attempt_tuples_per_theorem']}, "
                f"theoretical_max={max_possible_attempts}."
            )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--disable-lean-workbook", action="store_true")
    p.add_argument("--max-workbook-rows", type=int, default=None)

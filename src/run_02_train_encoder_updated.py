...
CONFIG = {
    ...
    "data": {
        ...
        "required_columns": [
            "dataset_name", "source_split", "theorem_id", "theorem_name", "state_pp", "tactic", "tactic_head", "sibling_group_id",
    ...
    "output": {
        ...
        "eval_predictions_file": "eval_predictions.jsonl",
...
def split_key_for_row(row: pd.Series) -> str:
    theorem_id = str(row.get("theorem_id", "unknown"))
    if CONFIG["data"].get("split_by") == "dataset_theorem_id":
...
def load_and_prepare_dataframe(tuple_dir: str | Path, limit_rows: Optional[int] = None) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    missing_columns_seen = set()

    for f in files:
        df = pd.read_parquet(f)
        for c in need:
            if c not in df.columns:
                missing_columns_seen.add(c)
                df[c] = None
        df = df[need]
        dfs.append(df)
    ...
    df["split_key"] = df.apply(split_key_for_row, axis=1)

    if df["sibling_group_id"].isna().any():
        missing = df["sibling_group_id"].isna()
        df.loc[missing, "sibling_group_id"] = [
            sha1_text(f"{a}:{b}")[:16] for a, b in zip(df.loc[missing, "split_key"], df.loc[missing, "state_hash"])
        ]
    ...
    stats = {
        ...
        "missing_columns_filled": sorted(missing_columns_seen),
        "num_theorems": int(df["theorem_id"].nunique()),
        "num_split_keys": int(df["split_key"].nunique()),
        "num_sibling_groups": int(df["sibling_group_id"].nunique()),
        "source_counts": df["dataset_name"].fillna("unknown").value_counts().to_dict() if "dataset_name" in df else {},
...
def split_by_theorem(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    ...
    split_keys = list(df["split_key"].dropna().unique())
    rng.shuffle(split_keys)
    n = len(split_keys)
    n_train = int(n * CONFIG["data"]["train_ratio"])
    n_valid = int(n * CONFIG["data"]["valid_ratio"])
    train_keys = set(split_keys[:n_train])
    valid_keys = set(split_keys[n_train:n_train + n_valid])
    test_keys = set(split_keys[n_train + n_valid:])

    splits = {
        "train": df.index[df["split_key"].isin(train_keys)].to_numpy(),
        "valid": df.index[df["split_key"].isin(valid_keys)].to_numpy(),
        "test": df.index[df["split_key"].isin(test_keys)].to_numpy(),
    }
    print("[split]", {k: int(len(v)) for k, v in splits.items()})
    return splits


def normalized_theorem_stem(name: Any) -> str:
    s = normalize_ws(name)
    s = re.sub(r"\d+", "<n>", s)
    s = re.sub(r"[_'.]+", "_", s)
    return s.lower()
...
def split_metadata(df: pd.DataFrame, splits: Dict[str, np.ndarray]) -> Dict[str, Any]:
    split_keys = {k: set(df.iloc[v]["split_key"].dropna().astype(str)) for k, v in splits.items()}
    theorem_ids = {k: set(df.iloc[v]["theorem_id"].dropna().astype(str)) for k, v in splits.items()}
    state_tactic = {
        k: set((df.iloc[v]["state_hash"].astype(str) + "\t" + df.iloc[v]["tactic"].astype(str)).tolist())
        for k, v in splits.items()
    }
    state_hashes = {k: set(df.iloc[v]["state_hash"].dropna().astype(str)) for k, v in splits.items()}
    stems = {k: set(df.iloc[v]["theorem_name"].map(normalized_theorem_stem)) for k, v in splits.items()}

    def overlap_count(groups: Dict[str, set], a: str, b: str) -> int:
        return len(groups[a] & groups[b])

    meta = {
        "split_by": CONFIG["data"].get("split_by"),
        "split_sizes_rows": {k: int(len(v)) for k, v in splits.items()},
        "split_sizes_keys": {k: len(split_keys[k]) for k in splits},
        "split_keys": {k: sorted(split_keys[k]) for k in splits},
        "theorem_ids": {k: sorted(theorem_ids[k]) for k in splits},
        "leakage_checks": {
            "split_key_overlap_train_valid": overlap_count(split_keys, "train", "valid"),
            "split_key_overlap_train_test": overlap_count(split_keys, "train", "test"),
            "split_key_overlap_valid_test": overlap_count(split_keys, "valid", "test"),
            "theorem_id_overlap_train_valid": overlap_count(theorem_ids, "train", "valid"),
            "theorem_id_overlap_train_test": overlap_count(theorem_ids, "train", "test"),
            "exact_state_tactic_overlap_train_valid": overlap_count(state_tactic, "train", "valid"),
            "exact_state_tactic_overlap_train_test": overlap_count(state_tactic, "train", "test"),
            "state_hash_overlap_train_valid": overlap_count(state_hashes, "train", "valid"),
            "state_hash_overlap_train_test": overlap_count(state_hashes, "train", "test"),
            "theorem_name_stem_overlap_train_valid": overlap_count(stems, "train", "valid"),
            "theorem_name_stem_overlap_train_test": overlap_count(stems, "train", "test"),
        },
    }
    print("[split:leakage]", json.dumps(meta["leakage_checks"], ensure_ascii=False))
    return meta
...
class SiblingCollator:
    ...
    def _sample_group(self, indices: List[int]) -> List[int]:
        ...
        if not self.train:
            return rows.sort_values(["tactic_rank"], ascending=[True]).index[: self.max_candidates].astype(int).tolist()
...
def id_to_result_type(idx: int) -> str:
    return RESULT_TYPES[int(idx)] if 0 <= int(idx) < len(RESULT_TYPES) else "Other"


def id_to_delta_bucket(idx: int) -> str:
    return DELTA_BUCKETS[int(idx)] if 0 <= int(idx) < len(DELTA_BUCKETS) else "invalid"
...
def collect_predictions(
    ...
    for bi, batch in enumerate(loader):
        ...
        result_pred = out["result_type_logits"].argmax(-1).detach().cpu().numpy()
        delta_pred = out["delta_bucket_logits"].argmax(-1).detach().cpu().numpy()
        row_ids = batch["row_ids"].detach().cpu().numpy()
        labels = batch["y"].detach().cpu().numpy()
        target = batch["utility_target"].detach().cpu().numpy()
        ranks = batch["tactic_rank"].detach().cpu().numpy()
        result_true = batch["result_type_label"].detach().cpu().numpy()
        delta_true = batch["delta_bucket_label"].detach().cpu().numpy()
        sibling_ids = batch["sibling_group_id"]

        for i in range(len(row_ids)):
            rows.append({
                "phase": phase_name,
                "row_id": int(row_ids[i]),
                "sibling_group_id": sibling_ids[i],
                "y": float(labels[i]),
                "tactic_rank": int(ranks[i]),
                "utility_target": float(target[i]),
                "validity_score": float(validity[i]),
                "utility_score": float(utility[i]),
                "result_type_true": id_to_result_type(int(result_true[i])),
                "result_type_pred": id_to_result_type(int(result_pred[i])),
                "delta_bucket_true": id_to_delta_bucket(int(delta_true[i])),
                "delta_bucket_pred": id_to_delta_bucket(int(delta_pred[i])),
            })
    return rows
...
def train_phase(
    ...
    train_loader = DataLoader(
        ...
        pin_memory=CONFIG["batching"]["pin_memory"] and device.type == "cuda",
...
def apply_args(args: argparse.Namespace) -> None:
    ...
    if args.max_eval_batches is not None:
        CONFIG["training"]["max_eval_batches"] = args.max_eval_batches
    if args.save_eval_predictions:
        CONFIG["output"]["save_eval_predictions"] = True
...
def main() -> None:
    ...
    if args.config_only:
        summary = {
            "tuple_dir": CONFIG["data"]["tuple_dir"],
        ...
        save_json(output_dir / "config_only_summary.json", summary)
    ...
    split_meta = split_metadata(df, splits)
    save_json(output_dir / CONFIG["output"]["split_file"], split_payload)
    save_json(output_dir / CONFIG["output"]["split_metadata_file"], split_meta)
    ...
    if CONFIG["output"].get("save_eval_predictions", True):
        pred_rows = collect_predictions(model, test_loader, device, "test", autocast_dtype, max_batches=None)
        write_jsonl(output_dir / CONFIG["output"]["eval_predictions_file"], pred_rows)

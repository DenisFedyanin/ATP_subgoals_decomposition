...
class DataConfig:
    ...
    additional_sft_data: List[str] = field(
        default_factory=lambda: [
            "outputs/domain_selected_whole_seq_and_sorry_datasets/domain_solved_from_sorry_skeletons.jsonl"
        ]
    )
    trust_additional_sft_data: bool = True
...
def imports_to_text(imports: Any) -> str:
    if imports is None or imports == "":
        return ""
    if isinstance(imports, list):
        names = [str(x).strip() for x in imports if str(x).strip()]
    else:
        raw = str(imports)
        if "\n" in raw:
    ...
    lines = []
    for name in names:
        lines.append(name if name.startswith("import ") else f"import {name}")
    return "\n".join(lines) + ("\n\n" if lines else "")


def strip_existing_proof_from_decl(formal: str) -> str:
    s = str(formal or "").strip()
    if not s:
        return s
...
def extract_code_fields(record: Dict[str, Any]) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    if full_code is None:
        full_code = nested_first_present(
            record,
            [
                ["output", "full_theorem_code"],
                ["trajectory", "full_theorem_code"],
                ["source_solution", "full_theorem_code"],
            ],
        )
    ...
    if formal_statement is None:
        formal_statement = nested_first_present(
            record,
            [
                ["input_context", "formal_statement"],
                ["source_job", "formal_statement"],
                ["source_job", "problem", "formal_statement"],
                ["problem", "formal_statement"],
                ["source_solution", "formal_statement"],
            ],
        )
    ...
    if proof_body is None:
        proof_body = nested_first_present(
            ...
            [
                ...
                ["solved_solution", "tactic_script"],
                ["solved_solution", "tactics"],
                ["reference_solution", "tactic_script"],
                ["reference_solution", "tactics"],
                ["reference_proof_trajectory", "tactic_script"],
                ["reference_proof_trajectory", "tactics"],
                ["solution", "tactic_script"],
                ["source_solution", "proof_body"],
                ["source_solution", "target_proof"],
            ],
        )

    header = first_present(record, ["imports", "header", "lean_header", "prelude"], None)
    if header is None:
        header = nested_first_present(record, [["problem", "imports"], ["source_job", "imports"], ["source_job", "problem", "imports"]], "")
    if header is None:
        header = ""
...
def build_prompt_completion(
    ...
    if formal_statement and proof_body:
        statement = normalize_ws(strip_existing_proof_from_decl(str(formal_statement)), data_cfg.strip_trailing_whitespace)
        if isinstance(proof_body, list):
            proof_raw = "\n".join(str(x) for x in proof_body)
        else:
            proof_raw = str(proof_body)
        proof = normalize_ws(proof_raw, data_cfg.strip_trailing_whitespace)
        ...
        if statement.endswith(":="):
            ...
            prompt = statement + " := by\n"
        header = normalize_ws(imports_to_text(header), data_cfg.strip_trailing_whitespace) if header else ""
...
def is_verified_record(record: Dict[str, Any]) -> bool:
    if record.get("verified") is True:
        return True
    if record.get("accepted_for_training") is True:
        return True
...
def load_additional_sft_examples(cfg: RunConfig) -> Tuple[List[SFTExample], Dict[str, int]]:
    counts: Dict[str, int] = {}
    if not cfg.data.additional_sft_data:
        return [], counts
    examples: List[SFTExample] = []
    reject_counts: Dict[str, int] = defaultdict(int)
    data_cfg = dataclasses.replace(cfg.data, require_verified=not cfg.data.trust_additional_sft_data)
    for path_str in cfg.data.additional_sft_data:
        p = Path(path_str)
        rows = read_jsonl(p)
        counts[f"loaded_additional_{p.name}"] = len(rows)
        counts[f"exists_additional_{p.name}"] = int(p.exists())
        for r in rows:
            ex, reason = record_to_sft_example(r, data_cfg, cfg.mixture, source_override="full_solved_direct")
            if ex:
                examples.append(ex)
            else:
                reject_counts[reason or "unknown"] += 1
    for k, v in reject_counts.items():
        counts[f"additional_rejected_{k}"] = v
    counts["accepted_additional_sft"] = len(examples)
    return examples, counts
...
def apply_cli_overrides(cfg: RunConfig, args: argparse.Namespace) -> RunConfig:
    ...
    if args.additional_sft_data:
        cfg.data.additional_sft_data.extend(args.additional_sft_data)
    if args.no_default_additional_sft_data:
        cfg.data.additional_sft_data = list(args.additional_sft_data or [])
    if args.base_model:
    ...
    if args.tokenize_dry_run:
        cfg.experiment.tokenize_dry_run = True
    if args.overwrite:
...
def build_and_save_datasets(cfg: RunConfig, tokenize: bool = True) -> Tuple[Any, Any, Any, Dict[str, Any]]:
    ...
    additional, additional_counts = load_additional_sft_examples(cfg)
    all_examples = mined + general + additional
    ...
    report = {
        ...
        "loaded": {**mined_counts, **general_counts, **additional_counts},
        ...
        "tokenization_skipped": not tokenize,
        "config": dataclass_to_dict(cfg),
    }

    if not tokenize:
        report.update({
            "num_tokenized_train": None,
            "num_tokenized_val": None,
            "num_tokenized_test": None,
            "tokenization_skipped_train": {},
            "tokenization_skipped_val": {},
            "tokenization_skipped_test": {},
        })
        write_json(out / "dataset_report.json", report)
        return train_ex, val_ex, test_ex, report
...
def config_summary(cfg: RunConfig) -> Dict[str, Any]:
    ...
    run06 = Path(cfg.data.run06_dir)
    warnings: List[str] = []
    if cfg.data.use_final_trajectories and not (run06 / cfg.data.final_trajectories_file).exists():
        warnings.append(f"missing_run06_final_trajectories: {run06 / cfg.data.final_trajectories_file}")
    if cfg.data.use_final_curriculum_items and not (run06 / cfg.data.final_curriculum_items_file).exists():
        warnings.append(f"missing_run06_final_curriculum_items: {run06 / cfg.data.final_curriculum_items_file}")
    if cfg.data.general_lean_data and not Path(cfg.data.general_lean_data).exists():
        warnings.append(f"missing_general_lean_data: {cfg.data.general_lean_data}")
    for path_str in cfg.data.additional_sft_data:
        if not Path(path_str).exists():
            warnings.append(f"missing_additional_sft_data: {path_str}")
    if not all_examples:
        warnings.append("no_sft_examples_loaded")
    return {
        "experiment": cfg.experiment.name,
        "created_at": now_ts(),
        "input_paths": {
            "run06_dir": cfg.data.run06_dir,
            "final_trajectories": str(run06 / cfg.data.final_trajectories_file),
            "final_curriculum_items": str(run06 / cfg.data.final_curriculum_items_file),
            "general_lean_data": cfg.data.general_lean_data,
            "additional_sft_data": cfg.data.additional_sft_data,
        },
        "loaded": {**mined_counts, **general_counts, **additional_counts},
        "accepted_examples_before_dedup": len(all_examples),
        "source_counts_before_dedup": dict(source_count(all_examples)),
        "model": dataclasses.asdict(cfg.model),
        "lora": dataclasses.asdict(cfg.lora),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--additional-sft-data", action="append", default=None)
    p.add_argument("--no-default-additional-sft-data", action="store_true")
    ...
    p.add_argument("--tokenize-dry-run", action="store_true")
...
def main() -> None:
    ...
    if cfg.experiment.config_only:
        ...
        summary_path = out / "config_only_summary.json"
        write_json(summary_path, summary)
        print(json.dumps({k: v for k, v in summary.items() if k != "config"}, indent=2, ensure_ascii=False))
        print(f"[config-only] summary written to {summary_path}")
        return

    tokenize = not cfg.experiment.dry_run or cfg.experiment.tokenize_dry_run
    train_ds, val_ds, test_ds, report = build_and_save_datasets(cfg, tokenize=tokenize)
